...
def resample_prices(close_prices, freq='M'):
    ...
    """
    prices_resampled = close_prices.resample(freq).ffill()
...
def get_top_n(prev_returns, top_n):
    ...
    for index,row in prev_returns.iterrows():
        top_stocks = top_stocks.append(prev_returns.loc[index].nlargest(top_n))
        top_stocks = top_stocks / top_stocks
        top_stocks = top_stocks.fillna(0)
...
def portfolio_returns(df_long, df_short, lookahead_returns, n_stocks):
    ...
    print('df_long is {}'.format(df_long))
    print('df_short is {}'.format(df_short))
    print('lookahead_returns is {}'.format(lookahead_returns))
    print('n_stocks is {}'.format(n_stocks))
    
    portfolio_returns = (df_long - df_short)*lookahead_returns
    portfolio_returns = portfolio_returns / n_stocks
...
def analyze_alpha(expected_portfolio_returns_by_date):
    ...
    t_value = stats.ttest_1samp(expected_portfolio_returns_by_date,0)[0]
...
# The p-value observed is 0.080 which is larger than alpha=0.05 indicating that we cannot reject the null hypothesis which indicates that the actual mean return from the signal may be 0 at the 5% level of significance. However if we were to raise the level of significance to 10% then the p-value will be less than alpha = 0.10 in which case we could reject the null hypothesis indicating  
